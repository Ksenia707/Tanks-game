...
from maps import Obstacle
from PowerUp import PowerUp1
...
def draw_lives(count):
    image = p.transform.scale(p.image.load('img/img_6.png'), (25, 25))
...
def menu():
    ...
    btn = p.image.load('img/img_5.png')
    ...
    while True:
        ...
        # изменяем размер при наведение курсора на кнопку
...
def map8():
    ...
    for s in range(3):
        block = Obstacle(x2, y2)
        all_sprites.add(block)
        y2 += 40

    for t in range(2):
        block = Obstacle(x3, y3)
        all_sprites.add(block)
        x3 += 110
